...
_Static_assert(sizeof(((wifi_config_t *) 0)->ap.password) >= 16,
               "AP password buffer too small for MAC-derived password");
...
esp_err_t wifi_manager_start_ap(void) {
    ...
    snprintf((char *) wifi_config.ap.password, sizeof(wifi_config.ap.password),
             "Fresnel%02X%02X%02X%02X%02X%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
